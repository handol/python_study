...
class MyHTTPRedirectHandler(urllib2.HTTPRedirectHandler):
	...
	def http_error_302(self, req, fp, code, msg, headers):
		...
		#self.fp.write(repr(seq_num) + ' ')
		#seq_num = seq_num + 1
		#f.write(repr(depth) + ' ')
		self.fp.write('loghandler 302 test')
		#f.write(repr(len(handle.info().__str__())) + '/')
		#f.write(repr(len(data)) + ' ')
		#f.write(repr(handle.code) + ' ')
		#f.write('%.3s' % (end - start) + ' ')
		#f.write(handle.geturl())
		self.fp.write('\n')

		return urllib2.HTTPRedirectHandler.http_error_302(self, req, fp, code, msg, headers)

		#super(MyHTTPRedirectHandler, self).http_error_302(req, fp, code, msg, headers)
...
class KunEngine(threading.Thread):
	...
	def __init__(self):
		...
		self.proxy_support = urllib2.ProxyHandler({'http':'http://127.0.0.1:8080'})
	...
	def do_request(self, url):
		...
		#self.header.add("HTTP_PHONE_NUMBER", MDN)
		#self.header.add("HTTP_MDN_INFO", MDN)
		...
		try:
			...
			print urllib2.localhost()

			print " ====================== "
			print " OUR REQUEST HEADER "				# Sending header print
			...
			req = urllib2.Request("http://m.dosirak.com", txdata, txheaders)
			handle = urllib2.urlopen(req)

			print '----------------------------------Fourth Request----------------------------------'
			req = urllib2.Request("http://m.dosirak.com/hihi", txdata, txheaders)
			handle = urllib2.urlopen(req)
...
#						data = handle.read()
#
#						parser = hparser.hparser(urls, debug=1)
#						parser.feed(data)
#						parser.close()
#						lists = parser.get_anchors()
#
...
#						fp.write(repr(depth) + ' ')
#						fp.write('REQSIZE ')
#						fp.write(repr(len(handle.info().__str__())) + '/')
#						fp.write(repr(len(data)) + ' ')
#						fp.write(repr(handle.code) + ' ')
#						fp.write('%.3s' % (end - start) + ' ')
#						fp.write(handle.geturl())
#						fp.write('\n')
#					except e:
#						continue
#
