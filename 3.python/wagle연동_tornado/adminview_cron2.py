...
def mytimelineAll(host='https://m2.wagle.me', mdn='01080805909', atoken='abcde'):
	...
	idstore = filestore("w.id.store")
	last_id = idstore.load_int()
	print "LAST", last_id	
	...
	if startid>0:
		#url = "%s/v3.0/statuses/myTimelineAll?pageSize=%d" % (host, PAGESIZE)
	...
	for ctents in contents:
		...
		## filter
		if cid <= last_id: continue	
		text = ctents['text']
		if text.find("Wagle")==-1: continue
