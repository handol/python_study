...
W_APPKEY = 'KZLv9SqivAwAyDR7R6HPn36STIs=#MA=='
P_APPKEY = '8R8A+JIT1AMxzSZZw/irpuRZnN0=#Ng=='
#P_APPKEY = '8R8A+JIT1AMxzSZZw/irpuRZnN0=#MTA='
APPKEY = P_APPKEY
TOKEN=''
USERSEQ='12345'
BOUNDARY="--$%handol%$--"
...
def basic_headers(req, mdn):
	...
	req.add_header("App-Agent", "Web;Web;Etc;petloves;1.0;Etc;LAN")
	req.add_header("AKey", APPKEY)



def proc_res(req, debug=0):
...
def moiminvite(host='https://m2.wagle.me', mdn='01080805909', gathID='10001173'):
	url = "%s/v3.0/community/%s/invite/friends?gathID=%s" % (host, gathID, gathID)
	#wterConts = wterConts.decode('euc-kr')
	req = urllib2.Request(url)
	basic_headers(req, mdn)	
...
if __name__ == "__main__":
	...
	for i in range(1):
		...
		P = '8R8A+JiT1AMxzSZZw/irpuRZnN0=#'
		APPKEY = P + base64.b64encode(str(i+6))
		t = login(host=HOST, mdn='01080808002', debug=1)
		if t != "": break
		time.sleep(2)
	
	#show(host=HOST, userseq='12345', atoken=TOKEN)

	#post(host=HOST, atoken=TOKEN, accessType='020', text='ÀÌÁ¦ ÅÂÇ³ÀÌ Áö³ª°£°É±î¿ä?')

	#moimlist(host=HOST, atoken=TOKEN)
	#moimpost(host=HOST, atoken=TOKEN, gathID='368', wterConts='¾ÆÀÚ ÇÑµ¹') 
