...
def mytimelineAll(host='https://m2.wagle.me', mdn='01080805909', atoken='abcde'):
	outfname = LOGIN_ID+".tweets"
	
	print "startid = ", startid
        
	if startid>0:
		url = "%s/v3.0/statuses/myTimelineAll?lineContentsId=%d&pageSize=50&direction=0" % (host, startid)
	else:
		url = "%s/v3.0/statuses/myTimelineAll?pageSize=50&direction=0" % (host)
