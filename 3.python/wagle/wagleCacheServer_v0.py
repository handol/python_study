...
class WagleDB:
	...
	def load_contents(self, uid, limit=1000):
		try:
			query = "select count(*) from limit by %d" % (limit)
			cursor.execute(query)
			rows = cursor.fetchall()
		except:
			return []
...
class WagleCache:
	...
	def add_contents_list(self):
		c_u_list = self.dbhander.load_all_contents()
