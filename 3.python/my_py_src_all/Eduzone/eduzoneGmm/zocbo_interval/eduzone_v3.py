...
class DataList:
	...
	def load(self, fname):
		...
		self.mean = Vector(self.data[0].dim())
		for v in self.data:
			self.mean += v
...
class Cluster:
	...
	def prn(self):
		print "GMM, %f, %f, %s" % (self.pi, self.radius, self.mean.str())
...
def doit(userfile, dirname, extname):
	...
	for file in files:
		...
		datalist = DataList()
		datalist.load(file)
		numData = len(datalist.data)
		if (numData<30):
			continue
		print "%d items" % (numData)
		print "%s" % (datalist.mean.str())
		clResult.totalItems = len(datalist.data)
		clResult.prn()
		...
		print "%s, %d, %d, %d, %d, %f" % \
			(basename, numcoms, clResult.numClusters, goodClsCount,  numData, clResult.rissanen)
		#break
