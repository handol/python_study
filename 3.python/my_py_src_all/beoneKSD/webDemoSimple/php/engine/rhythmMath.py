...
class ksdModel:
	...
	def	diffToCenter(self, y):
		ydiff = VectorSubtraction(y, self.centerVec)
		distY = KsdDistance( ydiff )
		return distY

	def ratioToCenter(self, y):
		distY = self.diffToCenter(y)
	...
	def getRatios(self):
		...
		for v in self.learnVectors:
			self.ratios.append( self.ratioToCenter(v) )
		return self.ratios
