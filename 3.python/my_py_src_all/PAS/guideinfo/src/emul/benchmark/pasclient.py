...
#----------------------------------------------------------------------------
def getSummaryStats(runTimeInfo, programRunTime, numberOfConnection, targetClients, countConFail, countTimeout, countActiveClose):
	...
	stats["ActiveClose"] = countActiveClose
...
def saveSumStats(sumStats):
	...
	fd = file("sumSummary.csv", "w")
 	line = "RunTime, Client, ReqPerConn, ConnSuccess, ConnFailure, Timeout, Request, AvgReqTime, RecvSizePerSec, AvgReqPerSec, ReqSleepTime, ActiveClose\n"
 	fd.writelines(line)
	...
	for s in sumStats:
		#try:
			...
			line = "%7d, %6d, %10d, %11d, %11d, %7d, %7d, %10f, %14d, %12d, %12f, %11d\n"  % \
				(s["RunTime"], s["Client"], s["ReqPerConn"], s["ConnSuccess"], s["ConnFailure"], s["Timeout"], s["Request"], AvgReqTime, RecvSizePerSec, AvgReqPerSec, RequestSleepTime, s["ActiveClose"])
...
#----------------------------------------------------------------------------
def getSummary(runTimeInfo, programRunTime, numberOfConnection, targetClients, countConFail, countTimeout, countActiveClose):
	global RequestPerConnecion
	global RequestSleepTime
...
if __name__ == "__main__":
	...
	if TargetClients > 0:
		...
		Steps = range(MinClients, MaxClients+1, StepClients);
		if Steps[len(Steps)-1] != MaxClients:
			Steps.append(MaxClients)

		for TargetClients in Steps:
			runTimeInfo = []

			if TargetClients < MaxClients:
				stats = startRequest(myreactor, runTimeInfo, UrlList, TargetClients, IntervalTime)

			else:
				stats = startRequest(myreactor, runTimeInfo, UrlList, TargetClients, LastTime)
