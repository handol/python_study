...
class _html2text(sgmllib.SGMLParser):
	...
	def handle_tag(self, tag, attrs, start):
		...
		## handol: DO NOT newline in <A> tag
		#if tag == "br" and start: self.o("  \n")
		if tag == "br" and start: 
			if not self.astack: self.o("  \n")
		...
		## handol: NO special handling for em, strong
		...
		if tag == "a":
			if start:
				...
				if attrs.has_key('href'): 
					self.linkList.append(attrs['href'])
...
class url2text:
	...
	def getAbsLinks(self):
		...
		self.links = map(lambda link: urlparse.urljoin(self.url, link), self.links)
		return self.links

	def getCmpUrl(self):
		orgurl = urlparse.urlsplit(self.url)
		print orgurl
		...
		for link in self.links:
			if link.startswith(cleanurl) and not link.startswith(self.url):
				print "ORG: %s\nCMP: %s" % (self.url, link)
				return link
		return ''
...
if __name__ == "__main__":
	if sys.argv[1:]:
		...
		if arg.startswith('http://'):
			...
			cmpU = url2text(u2t.getCmpUrl())
			cmpU.getText()
			
			cmpTwoTexts(u2t.text, cmpU.text)
			...
			data = html2text(text.decode(encoding))
