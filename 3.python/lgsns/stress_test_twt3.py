...
def table_query_loop(cursor):
	for i in range(100):
	        query = "select userName, email, phoneNum, groupID from twt_address where userID='%s' order by regDate desc"
...
query_form = "select userName, email, phoneNum, groupID from twt_address where userID='%s' order by regDate desc"
