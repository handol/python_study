...
query_form = "select userid, contentID, contentKind, content, regdate from twt_contents where openType=1 and isDel=0 and userID in (select followerUserID as userID from twt_follower_list where UserID  = '%s' and state = 1 and isDel=0 ) order by regDate desc limit 0, 20"
